...
def get_pos(mouse, rot_mat):
    return (np.matmul(rot_mat, [mouse.getPos()[0], mouse.getPos()[1]]))
...
myMouse = event.Mouse(visible=False, newPos=None, win=mywin)
...
for i in range(len(trials.trial_num)):
    ...
    is_home = False
    while is_home == False:
        if home.contains(get_pos(myMouse, no_rot)):
            is_home = True
        current_pos = get_pos(myMouse, no_rot)
    ...
    while move_clock.getTime() <= timeLimit:
        # Run trial
        current_pos = get_pos(myMouse, rot_mat)
        update_pos(current_pos, int_cursor)
        update_pos(calc_target_pos(
            trials.target_pos[i], trials.target_amp[i]), target)
        mywin.flip()
